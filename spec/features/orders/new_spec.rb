...
RSpec.describe("New Order Page") do
  describe "When I check out from my cart" do
    ...
    it 'can see the discount if there is one' do
      ...
      within "#order-item-#{@paper.id}" do
        ...
        expect(page).to have_content("$#{@paper.price}")
        expect(page).to have_content("2")
        expect(page).to have_content("$20")#new discount price from $40
        expect(page).to have_content("Fall Discount at 50% off")
      end
    end

    it 'can see the no discount message when there is no discount' do
      visit "/cart"
      click_on "Checkout"
      within "#order-item-#{@paper.id}" do
        expect(page).to have_content("Sorry No Discount Available")
      end
    end
  end
end
